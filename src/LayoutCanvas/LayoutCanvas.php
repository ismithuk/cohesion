...
class LayoutCanvas implements LayoutCanvasElementInterface, \JsonSerializable {
    ...
    foreach ($this->iterateCanvas() as $element) {
      ...
      if ($element->getModel()) {
        foreach ($element->getModel()->getValues() as $key => $value) {
          ...
          elseif ($withMediaReferences && is_string($value) && preg_match(self::MEDIA_REFERENCE_REGEX, $value)) {
          ...
          elseif ($withLinksReferences && is_string($value) && preg_match(self::LINK_REFERENCE_REGEX, $value)) {
            if ($reference = $this->decodeLinkReference($value)) {
              $references[] = [
                'entity_type' => $reference[0],
                'entity_id' => $reference[1],
              ];
            }
          }
          elseif ($withLinksReferences && (is_object($value) || is_array($value))) {
            $this->addLinksToReferences($value, $references);
          }
        }
      }
  ...
  /**
   * Traverses $value and adds links references to $references array.
   *
   * @param mixed $value
   ...
   */
  protected function addLinksToReferences($value, array &$references) {
    ...
    elseif (is_object($value) || is_array($value)) {
      foreach ($value as $item) {
        $this->addLinksToReferences($item, $references);
      }
    }
